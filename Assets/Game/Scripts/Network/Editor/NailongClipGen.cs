...
namespace HagenDa.EditorTools
{
    ...
    public static class NailongClipGen
    {
        ...
        public static void SaveIdlePoseAsset(string assetPath = "Assets/Game/Animation/Nailong_IdlePose.anim",
            float shoulderDown = 0.5f, float armForward = 0.55f, float armIn = 0.10f,
            float elbowBend = 0.35f, float spineLean = 0.12f, float kneeBend = 0.10f)
        {
            System.IO.Directory.CreateDirectory("Assets/Game/Animation");
            var clip = CreateIdlePose(0.2f, shoulderDown, armForward, armIn, elbowBend, spineLean, kneeBend);
            AssetDatabase.CreateAsset(clip, assetPath);
            var ctrl = AssetDatabase.LoadAssetAtPath<UnityEditor.Animations.AnimatorController>("Assets/Game/Characters/nailong/nailong.controller");
            if (ctrl != null)
            {
